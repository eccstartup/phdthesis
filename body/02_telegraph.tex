...
\right.
\end{equation}
式中 $p \in \mathbb{R}^n$ 和 $q \in \mathbb{R}^n$ 为所求函数, $H=H(q,p)$ 是系统的哈密尔顿量.在实际应用中, $n$ 为该哈密尔顿系统的自由度, $q$ 与 $p$ 分别对应系统的广义位移和广义动量.在相空间 $\mathbb{R}^{2n}$ 上的标准辛形式是如下的二微分形式
\begin{equation*}
...
首先选取等间距的时间格点来进行半离散.取 $0=x_0<x_1<\cdots<x_{N-1}<x_N=1$ 式中 $x_i=i\Delta x,~i = 1,2,\cdots,N$.
...
类似地,可以得到 $n$ 维电报方程 \eqref{eq:kg2d} 的阶条件, 如果取等间距的时间网格划分
...
为了得到 $\Delta x$ 的阶,固定一个较小的 $\Deltat$, 然后相应地变化 $\Delta x$. 结果在表 \ref{tab:dx1} 中给出. 这里取 $\Delta t=0.001$ 和 $T=2$.
